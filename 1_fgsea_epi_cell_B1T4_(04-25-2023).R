...
DEG <- read.csv("epi_cell_BtoT_cluster_fgesa.csv", sep = ',')
DEG_1 <- DEG %>% select(B1T4_names, B1T4_score, B1T4_pvals)
DEG_1 %>% filter(B1T4_pvals < 0.05)
DEG_1$gene <- DEG_1$B1T4_names
...
fwrite(fgseaResTidy, file="Epi_cell_B1T4_GSEA_results_KEGG.csv", sep=",", sep2=c("", " ", ""))
...
fwrite(fgseaResTidy_2, file="Epi_cell_B1T4_GSEA_results_REACTOME.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_2 = read.csv('Epi_cell_B1T4_GSEA_results_REACTOME.csv', sep=",")
...
fwrite(fgseaResTidy_5, file="Epi_cell_B1T4_GSEA_results_GOBP.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_5 = read.csv("Epi_cell_B1T4_GSEA_results_GOBP.csv", sep = ",")
...
ggplot(fgseaResTidy_5, aes(reorder(pathway, NES), NES)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="B1T4vs.others GOBP pathways") + 
  theme_minimal()
