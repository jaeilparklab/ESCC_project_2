...
DEG <- read.csv("T_cell_MtoT_cluster_fgesa.csv", sep = ',')
DEG_1 <- DEG %>% select(M3toT4_names, M3toT4_score, M3toT4_pvals)
DEG_1 %>% filter(M3toT4_pvals < 0.05)
DEG_1$gene <- DEG_1$M3toT4_names
...
fwrite(fgseaResTidy, file="T_M3T4_GSEA_results_KEGG.csv", sep=",", sep2=c("", " ", ""))
...
fwrite(fgseaResTidy_2, file="T_M3T4_GSEA_results_REACTOME.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_2 = read.csv('T_M3T4_GSEA_results_REACTOME.csv', sep=",")

plotEnrichment(msigdbr_list_2[["REACTOME_INTERFERON_ALPHA_BETA_SIGNALING"]],
               ranks_1) + labs(title="REACTOME_INTERFERON_ALPHA_BETA_SIGNALING")
...
fwrite(fgseaResTidy_5, file="T_M3T4_GSEA_results_GOBP.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_5 = read.csv("T_M3T4_GSEA_results_GOBP.csv", sep = ",")
...
ggplot(fgseaResTidy_5, aes(reorder(pathway, NES), NES)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="M3T4vs.others_GOBP Hallmark pathways") + 
  theme_minimal()
