...
DEG <- read.csv("epi_cell_MtoT_cluster_fgesa.csv", sep = ',')
...
fwrite(fgseaResTidy, file="epi_M3T4_GSEA_results_KEGG.csv", sep=",", sep2=c("", " ", ""))
...
fwrite(fgseaResTidy_2, file="epi_M3T4_GSEA_results_REACTOME.csv", sep=",", sep2=c("", " ", ""))
...
fwrite(fgseaResTidy_5, file="epi_M3T4_GSEA_results_GOBP.csv", sep=",", sep2=c("", " ", ""))

ggplot(fgseaResTidy_5, aes(reorder(pathway, NES), NES)) +
  geom_col(aes(fill=padj<0.05)) +
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="WTvs.Wnt5a Hallmark pathways") + 
  theme_minimal()
