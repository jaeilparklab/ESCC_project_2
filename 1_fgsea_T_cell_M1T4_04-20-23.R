...
DEG_1 <- DEG %>% select(M1toT4_names, M1toT4_score, M1toT4_pvals)
DEG_1 %>% filter(M1toT4_pvals < 0.05)
DEG_1$gene <- DEG_1$M1toT4_names
...
fwrite(fgseaResTidy, file="T_M1T4_GSEA_results_KEGG.csv", sep=",", sep2=c("", " ", ""))
...
fwrite(fgseaResTidy_2, file="T_M1T4_GSEA_results_REACTOME.csv", sep=",", sep2=c("", " ", ""))
#fgseaResTidy_2 = read.csv('T_M1T4_GSEA_results_REACTOME.csv', sep=",")
...
fwrite(fgseaResTidy_5, file="T_M1T4_GSEA_results_GOBP.csv", sep=",", sep2=c("", " ", ""))
fgseaResTidy_5 = read.csv("T_M1T4_GSEA_results_GOBP.csv", sep = ",")
...
ggplot(fgseaResTidy_5, aes(reorder(pathway, NES), NES)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
       title="M1T4vs.others GOBP pathways") + 
  theme_minimal()
